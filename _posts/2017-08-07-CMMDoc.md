...
```
* text：文字，可选网络加载文字，加载诸如更新日志一类的东西。文字中可以使用占位符太指代特定数据，占位符有如下几种：
  * #mcversion#：MC的版本
  * #fmlversion#：FML的版本
  * #mcpversion#：MCP的版本
...
当然，还有我们最重要的action参数，action下必须要有type参数，可以传入`openLink, openGui, quit, refresh, connectToServer, loadWorld, openFolder`。
